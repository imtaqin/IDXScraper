...
import { FinancialReportRDF} from '../models/index.js';
...
const saveFinancialReportData = async (data) => {
    const downloadFolder = path.resolve('./downloads/FinancialReportsRDF');
...
console.log()
                await FinancialReportRDF.create({
                    EmitenCode : attachment.Emiten_Code,
                    NamaEmiten: item.NamaEmiten,
                    Attachment: downloadFolder+"/"+filePath,
                    Tanggal: convertToTanggalBulan(attachment.File_Modified) // Convert the string to a Date object
                });
...
const getFinanceReportRDF = async () => {
    let options = new chrome.Options();
    ...
    try {
        const url = 'https://www.idx.co.id/primary/ListedCompany/GetFinancialReport?indexFrom=1&pageSize=12&year=2023&reportType=rdf&EmitenType=s&periode=tw1&kodeEmiten=&SortColumn=KodeEmiten&SortOrder=asc';
        await driver.get(url);
